const DomesticHotel = require("../models/Hotel/DomesticHotel");

exports.domesticHotelCreate = async (req, res) => {
  try {
    const postHotel = new DomesticHotel(req.body);

    const result = await postHotel.save();
    console.log(result);
    res.status(200).json({
      message: "Successfully domestic hotel details posted.",
      result,
    });
  } catch (error) {
    console.log(error);
...
exports.getPackageForAddPage = async (req, res) => {
  try {
    const getPackage = await DomesticHotel.find({}).sort({ createdAt: -1 });
    res
      .status(200)
      .json({
        message: "Successfully get all domestic hotel data",
        getPackage,
      });
  } catch (error) {
...
exports.getSpecificPackage = async (req, res) => {
  try {
    ...
    const getPackage = await DomesticHotel.findOne({ _id: id });
...
exports.deleteHotelPackage = async (req, res) => {
  try {
    ...
    const getPackage = await DomesticHotel.deleteOne({ _id: id });
...
exports.updateHotelPackage = async (req, res) => {
  try {
    ...
    const updateHotelInfo = await DomesticHotel.updateMany(
      { _id: id },
      {
        $set: body,
      },
